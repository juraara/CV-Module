...
int main()
{
    ...
    string outputpath = parent_path + "Output/gton_eye_closure_perdiff.txt";
    ...
    for (int k = 2; k < y_angle.size(); k++)
    {
        ...
        for (int h = 0; h < eye_closure_perdiff.size(); h++)
        {
            ...
            for (int i = 0; i < folder_path.size(); i++)
            {
                ...
                cout << path << endl;
                builder.BuildComputerVision(path, "0x", 80.0, 0, eye_closure_perdiff.at(h), "GTON");
                // builder.BuildComputerVision(path, "0x", 80.0, 0, 50.0, "GTON");
                ...
                verification_res.push_back(_verify(blink_frameno, builder._GTON.blink_in_frameno));
