...
public:
    ...
    void detectBlink(Mat& frame, Rect& eye, Rect& iris)
    {
        ...
        GaussianBlur(gray, blur, Size(9, 9), 0); // blur image
        Mat thresh;
        threshold(blur, thresh, minthresh, 255, THRESH_BINARY_INV); // convert to binary image
        ...
        int src_h = thresh.rows * (1 - (CROP_PERCENT * 2));
        Mat crop = thresh(Rect(x, y, src_w, src_h)); // crop side to remove eyebrows etc.
        frame = frame(Rect(x, y, src_w, src_h));
        ...
        frame = frame(Rect(0, 0, upper_w, upper_h));

        // <Image Pre-processing Ends Here>
        //   1. This part is where other blink detection methods starts to differ in algorithm

        // Find contours in upper half of cropped frame
        vector<vector<Point> > contours;
        vector<Vec4i> hierarchy;
        hierarchy.clear();
        contours.clear();
        findContours(upper, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0));
        ...
        {            
            ...
            // Check if Largest Contour Area is 0.
            //   1. If largest contour area is 0, mark eyestate as closed state
            //   2. If largest contour area is not 0, mark eyestate as open 
