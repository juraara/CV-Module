...
public:
    ...
    void detectBlink(Mat& frame, Rect& eye, Rect& iris)
    {
        ...
        // Get Lower Half of Cropped Frame
        int lower_w = crop.size().width;
        int lower_h = crop.size().height - upper.size().height;
        Mat lower = crop(Rect(0, upper.size().height, lower_w, lower_h)); // get upper half of image

        // Calculate Histogram of Upper and Lower Half
        int histSize = 256;
        float range[] = { 0, 256 }; // the upper boundary is exclusive
        const float* histRange[] = { range };
        bool uniform = true, accumulate = false;
        Mat hist_up, hist_lo;
        calcHist(&upper, 1, 0, Mat(), hist_up, 1, &histSize, histRange, uniform, accumulate); // get histogram
        calcHist(&lower, 1, 0, Mat(), hist_lo, 1, &histSize, histRange, uniform, accumulate); // get histogram

        // Compare Upper and Lower Number of Pixels
        float wpixels_up = hist_up.at<float>(255);
        float wpixels_lo = hist_lo.at<float>(255);
        
        prev_eyestate = eyestate;
        if (wpixels_up < wpixels_lo)
        {
            eyestate = 1;
        }
        else if (wpixels_up > wpixels_lo)
        {
            eyestate = 0;
        }
